...
void InitializationCell(int M, int N, double a, double b, double c, double d, double hx, double hy, double omega, Cell** cell_pp);
...
void PrintCellVelocity_u(int M, int N, Cell** cell_pp);
void PrintCellVelocity_v(int M, int N, Cell** cell_pp);
...
double VelocityFunction_u(double x, double y, double omega);
double VelocityFunction_v(double x, double y, double omega);
...
int main()
{
	...
	InitializationCell(M, N, a, b, c, d, hx, hy, omega, cell_pp);
	...
	PrintCellVelocity_u(M, N, cell_pp);
	PrintCellVelocity_v(M, N, cell_pp);
...
void InitializationCell(int M, int N, double a, double b, double c, double d, double hx, double hy, double omega, Cell** cell_pp)
{
	for (int j = 0; j < M; j++)
	{
		for (int i = 0; i < N; i++)
		{
			...
			cell_pp[j][i].u = VelocityFunction_u(cell_pp[j][i].cen_x, cell_pp[j][i].cen_y, omega);
			cell_pp[j][i].v = VelocityFunction_v(cell_pp[j][i].cen_x, cell_pp[j][i].cen_y, omega);
		}
	}
}
...
void PrintCellVelocity_u (int M, int N, Cell** cell_pp)
{
	ofstream OutFile("Data\\Cell Velocity\\u.txt");
...
void PrintCellVelocity_v(int M, int N, Cell** cell_pp)
{
	ofstream OutFile("Data\\Cell Velocity\\v.txt");
